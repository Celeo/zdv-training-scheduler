...
auth_resp=$(xh --ignore-stdin -p b https://api.backblazeb2.com/b2api/v3/b2_authorize_account -A basic -a $B2_KEY_ID:$B2_AUTH_KEY)
bucket_id=$(echo $auth_resp | jq -r .apiInfo.storageApi.bucketId)
auth_token=$(echo $auth_resp | jq -r .authorizationToken)

upload_loc_resp=$(xh --ignore-stdin https://api005.backblazeb2.com/b2api/v2/b2_get_upload_url bucketId==$bucket_id "Authorization:$auth_token")
...
upload_resp=$(xh --ignore-stdin post $upload_url @$1 "Authorization:$upload_auth_token" "Content-Type:binary/octet-stream" "X-Bz-File-Name:$1_$(date +%s)" "X-Bz-Content-Sha1:$checksum")
